                ...
                return (
                    <Link
                        ...
                        style={{
                            ...
                            background: isActive ? "linear-gradient(90deg, rgba(0,212,255,0.12) 0%, rgba(0,212,255,0.02) 100%)" : "transparent",
                            border: "1px solid transparent",
                            borderLeft: isActive ? "3px solid var(--cyan, #00d4ff)" : "3px solid transparent",
                            color: isActive ? "#fff" : "var(--text-muted, #4a5568)",
                            boxShadow: isActive ? "inset 1px 0 15px rgba(0,212,255,0.05)" : "none"
                        }}
                        onMouseEnter={(e) => {
                            if (!isActive) {
                                (e.currentTarget as HTMLAnchorElement).style.color = "var(--text-primary, #e8edf2)";
                                (e.currentTarget as HTMLAnchorElement).style.background = "rgba(255,255,255,0.03)";
                                (e.currentTarget as HTMLAnchorElement).style.borderLeft = "3px solid rgba(255,255,255,0.1)";
                                const iconSpan = e.currentTarget.querySelector('.nav-icon') as HTMLSpanElement;
                                if (iconSpan) iconSpan.style.color = "var(--text-primary, #e8edf2)";
                            }
                        }}
                        onMouseLeave={(e) => {
                            if (!isActive) {
                                (e.currentTarget as HTMLAnchorElement).style.color = "var(--text-muted, #4a5568)";
                                (e.currentTarget as HTMLAnchorElement).style.background = "transparent";
                                (e.currentTarget as HTMLAnchorElement).style.borderLeft = "3px solid transparent";
                                const iconSpan = e.currentTarget.querySelector('.nav-icon') as HTMLSpanElement;
                                if (iconSpan) iconSpan.style.color = "var(--text-muted, #4a5568)";
                            }
                        }}
                    >
                        <span className="nav-icon" style={{
                            color: isActive ? "var(--cyan, #00d4ff)" : "var(--text-muted, #4a5568)",
